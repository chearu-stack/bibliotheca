...
def build():
    ...
    site_js.write_text(site_js.read_text(encoding="utf-8") + 'const contextParams = new URLSearchParams(window.location.search); const requestedHall = contextParams.get("hall"); showHall(requestedHall === "poetry" ? "poetry" : "prose"); const requestedBook = decodeURIComponent(location.hash.slice(1)); if (requestedBook) { const hall = document.getElementById(requestedHall === "poetry" ? "poetry-hall" : "prose-hall"); const card = Array.from(hall.querySelectorAll("details.book-card")).find(item => item.id === requestedBook); if (card) { card.open = true; card.scrollIntoView({ block: "start" }); } }\n', encoding="utf-8")
